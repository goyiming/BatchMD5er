...
def convert_to_pinyin():
    ...
    if hanzi_text:
        ...
        pinyin_result = ''
        for line in lines:
            pinyin_text = pinyin(line, style=PinyinStyle.NORMAL)  # 使用 PinyinStyle.NORMAL
            processed_pinyin = ''
...
bottom_frame.grid(row=3, column=0, columnspan=3, sticky="ew", padx=10, pady=5)
bottom_label = ttk.Label(bottom_frame, text="o<) 软件说明：根据输入汉字转换成首字母大写拼音，加入固定字符，通过md5形式进行加密得到结果。", 
                         style="Info.TLabel", font=("Arial", 12))
bottom_label.pack(fill="x", expand=True)

# 左侧汉字输入框
hanzi_frame = ttk.LabelFrame(root, text="汉字", style="Card.TLabelframe")
hanzi_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
hanzi_entry = tk.Text(hanzi_frame, height=15, width=35)
...
# 中间拼音输入框
pinyin_frame = ttk.LabelFrame(root, text="拼音", style="Card.TLabelframe")
pinyin_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
pinyin_entry = tk.Text(pinyin_frame, height=15, width=35)
...
# 右侧转换结果显示框
result_frame = ttk.LabelFrame(root, text="转换结果", style="Card.TLabelframe")
result_frame.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
result_text = tk.Text(result_frame, height=15, width=35)
...
# 固定字符设置区域
fixed_chars_frame = ttk.LabelFrame(root, text="基础设置", style="Card.TLabelframe")
fixed_chars_frame.grid(row=1, column=0, columnspan=3, padx=10, pady=5, sticky="ew")

# 创建内部框架用于居中内容
inner_frame = ttk.Frame(fixed_chars_frame)
inner_frame.pack(expand=True)

# 创建下拉选择框标签
convert_label = ttk.Label(inner_frame, text="转换拼音方法：")
convert_label.pack(side=tk.LEFT, padx=4)

# 创建下拉选择框
convert_options = ttk.Combobox(inner_frame, values=["首字母大写", "全大写", "全小写"])
convert_options.current(0)  # 设置默认选中第一个选项
convert_options.pack(side=tk.LEFT, padx=4)

convert_label = ttk.Label(inner_frame, text="固定字符设置：")
convert_label.pack(side=tk.LEFT, padx=4)

# 创建固定字符输入框
fixed_chars_entry = ttk.Entry(inner_frame, width=20)
...
fixed_chars_button = ttk.Button(inner_frame, text="更新固定字符", command=update_fixed_chars, style="Accent.TButton")
fixed_chars_button.pack(side=tk.LEFT, padx=5)

# 添加注释说明居中效果
# 注意：此处使用内部框架并设置expand=True来实现整体居中效果

# 按钮
button_frame = ttk.Frame(root)
button_frame.grid(row=2, column=0, columnspan=3, pady=5)
# 转换按钮
convert_button = ttk.Button(button_frame, text="转换", command=convert_to_pinyin, style="Accent.TButton")
convert_button.pack(side=tk.LEFT, padx=4)

# 加密按钮
encrypt_button = ttk.Button(button_frame, text="加密", command=encrypt_name, style="Accent.TButton")
...
# 结果转大写按钮
uppercase_button = ttk.Button(button_frame, text="结果转大写", command=convert_to_uppercase, style="Accent.TButton")
